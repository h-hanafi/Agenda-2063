...
Model %>% group_by(Year) %>% summarize(cor = cor(GDP_Growth,Unemployment, use = "complete.obs")) %>% 
  ggplot(aes(Year,cor)) + 
...
Model %>% group_by(Country_Name) %>% summarize(cor = cor(GDP_Growth,Unemployment, use = "complete.obs")) %>% 
...
fit_Model_Unemployment <- list(lm = fit_lm_Model_Unemployment, knn = fit_knn_Model_Unemployment, xgb = fit_xgb_Model_Unemployment, rf = fit_rf_Model_Unemployment)
